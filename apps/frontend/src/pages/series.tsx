...
export function Series() {
  ...
  const definition =
    seriesDefinitions.find(({ key }) => key === selectedKey) ??
    seriesDefinitions[0];
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (selectedKey === "years") await ensureSession();
      await create({
        key: selectedKey,
        year: parsedYear,
        value: parsedValue,
      ...
      setMessage(
        "The observation could not be saved. Verify the service connection and year uniqueness.",
      );
    }
  }
    ...
    <section className="space-y-6">
      <header>
        <h1 className="text-4xl font-bold tracking-tight">
          Year and TGI observations
        </h1>
        <p className="mt-2 max-w-3xl text-muted-foreground">
          Record observations with the form and render the selected Atlas
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(280px,0.8fr)_minmax(0,1.7fr)]">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Each year is unique within its collection.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form className="space-y-4" onSubmit={submit}>
              ...
              >
                <Plus />
                {createState.isLoading ? "Saving…" : "Save observation"}
              </Button>
            </form>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="flex-row items-start justify-between gap-3">
            <div>
              ...
              <CardDescription>
                Atlas MongoDB Cloud · <code>{definition.collection}</code>
              </CardDescription>
            </div>
            <Button
          ...
          <CardContent>
            {list.isError && (
              <Alert className="mb-4" variant="destructive">
                <AlertDescription>
                  This data source is unavailable.
                </AlertDescription>
              </Alert>
            )}
            <ObservationTable
          ...
          {!loading && !rows.length && (
            <TableRow>
              <TableCell
                ...
                colSpan={requiresValue ? 2 : 1}
              >
                No observations in this collection.
              </TableCell>
            </TableRow>
          )}
        </TableBody>
